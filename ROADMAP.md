...
## ネストしたオブジェクトの表示方法の改善（アイデア）

現状、カード内で `dimensions` や `warehouseLocation` のような
「オブジェクトの中にさらにオブジェクトが入っている項目」は、
`{...} 編集` という折りたたみボタンで表示され、クリックするまで
中身が見えない。

これはユーザーが「見るだけ」でも一手間かかってしまい、
「ツールが構造判断を先回りして隠してしまう」という点で、
JSON Deckの基本方針（ユーザー自身がデータを把握・判断できる状態を保つ）
とややズレている。

### 改善案
- 折りたたみボタンではなく、中身をJSON形式のプレーンテキストとして
  常時展開し、コードブロック風の枠（等幅フォント、薄い背景色）の中に
  表示する
- 「見る」はこの表示で完結させ、「編集」は引き続き既存の編集ボタンから行う
  （表示と編集の役割を分ける）
- 中身の構造（配列を含む/含まない等）によらず同じ方式で対応できるため、
  過剰な作り込みをせずに済む

きっかけ：商品データ（productId, tags, dimensions等を含む）を貼り付けた際、
tagsはタグとして正しく表示された一方、dimensions・warehouseLocationは
折りたたまれて中身が見えず、使い勝手の面で改善余地があると判明した。

## 「{ }」ボタンのボタン操作化（アイデア・一度実装し保留）

### 背景
「＋項目」ボタンは、値を配列に変換した後、ボタン操作だけで下位項目を
組み立てられる（生JSONを手書きする必要がない）。一方「{ }」ボタンは、
値をオブジェクトに変換したい場合に、生JSON編集モーダルを開いて
手書きさせる仕様になっており、この非対称性が「結果さえ同じであれば、
人間が手でJSONを書くプロセスは避けるべき」という設計方針と噛み合って
いなかった。

### 試みた実装と、そこで判明した課題
「{ }」ボタンを押すと、非enumerableなマーカー（__addFieldUI）付きの
空オブジェクトに変換し、renderNestedFieldを再利用したフィールド追加UI
を表示する形で一度実装した。

実際に動かしたところ、以下の問題が生じた：

- 新規追加した下位フィールド（例: テスト、TEST）が、親フィールド
  （例: PRODUCTID）に属する項目として視覚的に区別されず、他の
  トップレベル項目（PRODUCTNAME、PRICE等）と同じ階層・同じ見た目で
  並んでしまい、どの項目がどこに属するのか分からなくなった
- 編集ボタンで開く内容にも、キー名や値が正しく反映されない表示崩れが
  見られた

「＋項目」（配列への変換）は独自のアコーディオン形式のUIを持っており
階層が視覚的に保たれるのに対し、「{ }」（オブジェクトへの変換）は
既存のrenderNestedFieldをそのまま流用したため、階層構造を表現する
専用のUIが無く、結果として「フラットに全部並んでいるように見える」
状態になってしまったと考えられる。

### 今後の方針
この機能は一旦ロールバックし、「{ }」ボタンは元の仕様（生JSON編集
モーダルを開く）に戻す。将来再挑戦する場合は、単にrenderNestedFieldを
流用するのではなく、「＋項目」の配列用UIと同様に、**親子関係が
一目でわかる専用の見た目**（インデント、枠線、アコーディオン等）を
新たに設計する必要がある。見た目の作り込みまで含めて相応の実装コスト
がかかることが分かったため、実際に「{ }ボタンの生JSON手書きで
困った」という具体的な場面が出てから再検討する。
